...
sealed class AIChatBaiDu(string appKey, string secretKey, string modelUri, Func<IHttpClient>? httpClientProvide) :
    WebApi(httpClientProvide), IAIChat
{
    ...
    public async Task<IAIChatContext> Create()
    {
        ...
        return response.GetValue<string>("access_token", false) is { } accessToken ?
            new AIChatContextBaiDu(accessToken, modelUri, HttpClientProvide) :
