...
public sealed record QueryBind<Property, Obj>
{
    ...
    public Property? Bind
    {
        get
        {
            var key = SearchViewerInfo<int>.GenerateQueryKey(PropertyAccess, LogicalOperator);
            return Info.QueryCondition.TryGetValue(key).Value is { CompareValue: { } v } ? (Property)v : default;
        }
    ...
    internal QueryBind(SearchViewerInfo<Obj> info, string propertyAccess, LogicalOperator logicalOperator)
    {
        Info = info;
