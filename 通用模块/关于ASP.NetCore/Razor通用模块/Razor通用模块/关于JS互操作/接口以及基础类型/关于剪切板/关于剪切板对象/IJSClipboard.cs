...
public interface IJSClipboard : IClipboard
{
    ...
    /// 读取剪切板的内容，
    /// 它可以返回复杂格式的内容，如图片等，
    /// 如果读取失败，则返回<see langword="null"/>
    /// </summary>
    /// <param name="withObjectURL">如果这个值为<see langword="true"/>，
    /// 在复制二进制内容时，会同时创建一个ObjectURL，它可用于图像预览等用途</param>
    /// <returns></returns>
    Task<IClipboardItems?> ReadObject(bool withObjectURL = false);
    #endregion
}
