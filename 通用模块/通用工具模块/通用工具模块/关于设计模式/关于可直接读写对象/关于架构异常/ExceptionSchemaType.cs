...
sealed class ExceptionSchemaType(string propertyName, Type propertyType, Type legalType, string? message = null) : ExceptionSchema(propertyName, message ?? $"属性{propertyName}的合法类型为{legalType}，但实际类型为{propertyType}，类型非法")
{
    #region 属性类型
    /// <summary>
    /// 获取该属性目前的类型，
    ...
    /// 获取该属性的合法类型
    /// </summary>
    public Type LegalType { get; } = legalType;
