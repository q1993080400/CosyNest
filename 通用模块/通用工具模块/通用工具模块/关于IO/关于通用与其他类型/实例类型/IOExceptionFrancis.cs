...
public sealed class IOExceptionFrancis(string path, string message) : IOException(message)
{
    ...
    /// <param name="path">发生异常的路径</param>
    /// <returns></returns>
    public static IOExceptionFrancis BecauseExist(string path)
        => new(path, $"{path}不存在，或者路径不合法");
    #endregion
    #region 抛出异常
    /// <summary>
    /// 检查一个路径，如果它不存在，则抛出异常
    ...
    /// <returns></returns>
    public static IOExceptionFrancis BecauseNotLegal(string path)
        => new(path, "该路径不是合法路径");
    #endregion
    #region 抛出异常
    /// <summary>
    ...
    /// <returns></returns>
    public static IOExceptionFrancis BecauseFileType(string path, IFileType fileType)
    ...
    /// <returns></returns>
    public static IOExceptionFrancis BecauseOccupied(string path, string? another = null)
