...
public static class FastRealize
{
    ...
    #region 释放时执行某些异步操作
    /// <summary>
    /// 获取一个实现<see cref="IAsyncDisposable"/>的锁，
    /// 它可以利用using语句自动完成某些操作，以免遗漏
    /// </summary>
    /// <param name="dispose">在锁被释放的时候，这个委托会被执行</param>
    /// <returns></returns>
    public static IAsyncDisposable AsyncDisposable(Func<ValueTask> dispose)
        => new LockAsync(dispose);
    #endregion
