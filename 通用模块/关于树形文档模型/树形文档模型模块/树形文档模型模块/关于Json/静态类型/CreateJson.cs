...
public static class CreateJson
{
    ...
    /// 它会使用一切可以使用的转换器去转换集合
    /// </summary>
    public static JsonConverter JsonCollection { get; } = new JsonConverterFactoryCollection();
    #endregion
    #region 创建元组转换器
    /// <summary>
    /// 创建一个转换器，
    /// 它可以用来转换元组
    /// </summary>
    public static JsonConverter JsonTuple { get; } = new JsonConverterFactoryTuple();
    #endregion
    #region 创建投影转换器
    /// <summary>
    ...
    /// </summary>
    /// <inheritdoc cref="JsonConverterMap{ConvertTo, Map}"/>
    /// <inheritdoc cref="JsonConverterMap{ConvertTo, Map}.JsonConverterMap(Func{ConvertTo, Map}, Func{Map, ConvertTo})"/>
    public static JsonConverter<ConvertTo> JsonMap<ConvertTo, Map>(Func<ConvertTo, Map> toMap, Func<Map, ConvertTo> fromMap)
        => new JsonConverterMap<ConvertTo, Map>(toMap, fromMap);
    #endregion
    #region 创建多态转换器
    /// <summary>
    /// 创建一个转换器，
    ...
    /// <inheritdoc cref="JsonConverterPolymorphic{Obj}.JsonConverterPolymorphic(IEnumerable{Assembly})"/>
    /// <inheritdoc cref="JsonConverterPolymorphic{Obj}"/>
    public static JsonConverter<Obj> JsonPolymorphic<Obj>(IEnumerable<Assembly>? assemblies = null)
        where Obj : class
        => new JsonConverterPolymorphic<Obj>(assemblies ?? [typeof(Obj).Assembly]);
    #endregion
    #region 创建IDirect转换器
    /// <summary>
